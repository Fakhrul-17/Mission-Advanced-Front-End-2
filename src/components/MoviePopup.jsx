...
function MoviePopup({
  ...
  // Handler untuk buka video player
  function handlePlay() {
    if (isSeries && episodeList.length > 0) {
      // Untuk series, mulai dari episode 1
      setCurrentEpisode(episodeList[0])
    }
    setShowPlayer(true)
  }

  // Handler klik episode di list
  function handlePlayEpisode(ep) {
    setCurrentEpisode(ep)
    setShowPlayer(true)
  }

  // Handler ganti episode dari VideoPlayer
  function handleEpisodeChange(ep) {
    setCurrentEpisode(ep)
  }
      ...
      >
        ...
        >
          ...
          <div className="relative">
            ...
            {/* Overlay Play Button di tengah poster */}
            <button
              onClick={handlePlay}
              className="absolute inset-0 flex items-center justify-center group hover:bg-black/40 transition-all"
            >
              <div className="w-16 h-16 md:w-20 md:h-20 rounded-full bg-white/90 flex items-center justify-center shadow-2xl transition-transform group-hover:scale-110 opacity-0 group-hover:opacity-100">
          ...
          <div className="p-4 md:p-5 flex flex-col gap-3 md:gap-4 -mt-2 relative">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                {/* TOMBOL PLAY - Sekarang berfungsi buka VideoPlayer */}
                <button
                  onClick={handlePlay}
                  className="w-11 h-11 md:w-12 md:h-12 rounded-full bg-white text-black flex items-center justify-center hover:bg-gray-100 active:scale-95 transition-all shadow-lg"
                  title="Putar"
                >
